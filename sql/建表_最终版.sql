...
--
-- Temporary view structure for view `v_光伏日发电量统计`
--

DROP TABLE IF EXISTS `v_光伏日发电量统计`;
/*!50001 DROP VIEW IF EXISTS `v_光伏日发电量统计`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `v_光伏日发电量统计` AS SELECT 
 1 AS `统计日期`,
 1 AS `设备编号`,
 1 AS `设备类型`,
 1 AS `安装位置`,
...
--
-- Temporary view structure for view `v_光伏设备异常汇总`
--

DROP TABLE IF EXISTS `v_光伏设备异常汇总`;
/*!50001 DROP VIEW IF EXISTS `v_光伏设备异常汇总`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `v_光伏设备异常汇总` AS SELECT 
 1 AS `设备编号`,
 1 AS `设备类型`,
 1 AS `安装位置`,
 1 AS `装机容量kWp`,
...
--
-- Temporary view structure for view `v_大屏_告警统计`
--

DROP TABLE IF EXISTS `v_大屏_告警统计`;
/*!50001 DROP VIEW IF EXISTS `v_大屏_告警统计`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `v_大屏_告警统计` AS SELECT 
 1 AS `统计小时`,
 1 AS `告警总次数`,
 1 AS `高等级告警数`,
 1 AS `中等级告警数`,
...
--
-- Temporary view structure for view `v_大屏_能源总览`
--

DROP TABLE IF EXISTS `v_大屏_能源总览`;
/*!50001 DROP VIEW IF EXISTS `v_大屏_能源总览`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `v_大屏_能源总览` AS SELECT 
 1 AS `统计时间`,
 1 AS `总用电量kWh`,
 1 AS `总用水量m3`,
 1 AS `总蒸汽消耗量t`,
...
--
-- Temporary view structure for view `v_大屏_配电网状态`
--

DROP TABLE IF EXISTS `v_大屏_配电网状态`;
/*!50001 DROP VIEW IF EXISTS `v_大屏_配电网状态`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `v_大屏_配电网状态` AS SELECT 
 1 AS `配电房编号`,
 1 AS `名称`,
 1 AS `电压等级`,
 1 AS `变压器数量`,
...
--
-- Temporary view structure for view `v_趋势_多周期对比`
--

DROP TABLE IF EXISTS `v_趋势_多周期对比`;
/*!50001 DROP VIEW IF EXISTS `v_趋势_多周期对比`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `v_趋势_多周期对比` AS SELECT 
 1 AS `能源类型`,
 1 AS `当日数值`,
 1 AS `当周数值`,
 1 AS `当月数值`,
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`jw`@`%`*/ /*!50003 TRIGGER `trg_逆变器效率异常标记_插入` BEFORE INSERT ON `光伏发电数据表` FOR EACH ROW BEGIN
    -- 校验条件：逆变器效率非空且低于85%（任务书明确要求）
    IF NEW.逆变器效率 IS NOT NULL AND NEW.逆变器效率 < 85 THEN
        -- 1. 更新光伏设备信息表的运行状态为“异常”
        UPDATE 光伏设备信息表
        SET 运行状态 = '异常',
            更新时间 = NOW()
        WHERE 设备编号 = NEW.设备编号;
        
        -- 2. 插入告警记录（编号格式：ALM-PV-设备编号，完全≤20字符）
        INSERT INTO 告警信息表 (
            告警编号,
            告警类型,
            关联设备编号,
            ...
            CONCAT('ALM-PV-', NEW.设备编号),  -- 纯标识格式，无时间戳（如ALM-PV-INV-100）
            '设备故障',
            NEW.设备编号,
            NOW(),
            '中',
            CONCAT('逆变器效率异常：', NEW.设备编号, '效率', NEW.逆变器效率, '%<85%'),
            '未处理',
            '逆变器效率≥85%',
            NOW()
        );
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`jw`@`%`*/ /*!50003 TRIGGER `trg_逆变器效率恢复标记_插入` BEFORE INSERT ON `光伏发电数据表` FOR EACH ROW BEGIN
    -- 校验条件：效率≥85% + 设备当前为异常状态
    IF NEW.逆变器效率 IS NOT NULL 
       AND NEW.逆变器效率 >= 85 
       AND EXISTS (
           SELECT 1 FROM 光伏设备信息表
           WHERE 设备编号 = NEW.设备编号
             AND 运行状态 = '异常'
       ) THEN
        -- 1. 更新设备状态为“正常”
        ...
        UPDATE 告警信息表
        SET 处理状态 = '已结案',
            告警内容 = CONCAT(告警内容, '【已恢复：', NEW.逆变器效率, '%】')
        WHERE 关联设备编号 = NEW.设备编号
          AND 告警类型 = '设备故障'
          AND 告警内容 LIKE '逆变器效率异常：%'
          AND 处理状态 = '未处理';
    END IF;
END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`jw`@`%`*/ /*!50003 TRIGGER `trg_逆变器效率异常标记_更新` BEFORE UPDATE ON `光伏发电数据表` FOR EACH ROW BEGIN
    -- 校验条件：逆变器效率非空且低于85%
    IF NEW.逆变器效率 IS NOT NULL AND NEW.逆变器效率 < 85 THEN
        -- 1. 更新光伏设备信息表的运行状态为“异常”
        UPDATE 光伏设备信息表
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`jw`@`%`*/ /*!50003 TRIGGER `trg_逆变器效率恢复标记_更新` BEFORE UPDATE ON `光伏发电数据表` FOR EACH ROW BEGIN
    -- 校验条件：效率从<85%更新为≥85% + 设备当前为异常
    IF NEW.逆变器效率 IS NOT NULL 
       AND NEW.逆变器效率 >= 85 
       AND (OLD.逆变器效率 IS NULL OR OLD.逆变器效率 < 85)
       AND EXISTS (
           SELECT 1 FROM 光伏设备信息表
           WHERE 设备编号 = NEW.设备编号
             AND 运行状态 = '异常'
...
INSERT INTO `光伏预测数据表` VALUES ('PF-TEST-001','BGD102','2026-01-01','09:00-10:00',120.00,140.00,16.20,'V1.2'),('PF-TEST-002','BGD102','2026-01-01','10:00-11:00',130.00,152.00,16.92,'V1.2'),('PF-TEST-003','BGD102','2026-01-01','11:00-12:00',140.00,165.00,17.86,'V1.2'),('PF-TEST-004','BGD102','2026-01-02','09:00-10:00',125.00,145.00,15.80,'V1.2'),('PF-TEST-005','BGD102','2026-01-02','10:00-11:00',135.00,157.00,16.30,'V1.2'),('PF-TEST-006','BGD102','2026-01-02','11:00-12:00',145.00,170.00,17.24,'V1.2'),('PF-TEST-007','BGD102','2026-01-03','09:00-10:00',130.00,152.00,16.50,'V1.2'),('PF-TEST-008','BGD102','2026-01-03','10:00-11:00',140.00,164.00,16.80,'V1.2'),('PF-TEST-009','BGD102','2026-01-03','11:00-12:00',150.00,178.00,17.65,'V1.2'),('PF-TEST-010','BGD102','2026-01-04','09:00-10:00',135.00,154.00,14.20,'V1.2'),('PF100','BGD102','2026-01-04','09:00-10:00',120.50,115.25,-4.36,'V1.2'),('PF101','BGD102','2026-01-04','10:00-11:00',135.75,142.50,4.74,'V1.2'),('PF102','BGD102','2026-01-04','11:00-12:00',152.25,165.80,8.18,'V1.2'),('PF103','BGD103','2026-01-04','09:00-10:00',85.20,80.50,-5.52,'V1.2'),('PF104','BGD103','2026-01-04','10:00-11:00',92.50,105.30,13.84,'V1.2');
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`jw`@`%`*/ /*!50003 TRIGGER `trg_变压器新设备台账同步` AFTER INSERT ON `变压器监测数据表` FOR EACH ROW BEGIN
    DECLARE v_配电房名称 VARCHAR(50);
    DECLARE v_exists INT DEFAULT 0;

    -- 检查设备是否已存在于台账
    SELECT COUNT(*) INTO v_exists
    FROM `设备台账数据表`
    WHERE `台账编号` = NEW.`变压器编号`;

    IF v_exists = 0 THEN
        -- 获取配电房信息
        SELECT `名称` INTO v_配电房名称 FROM `配电房信息表` WHERE `配电房编号` = NEW.`配电房编号`;
        ...
        INSERT INTO `设备台账数据表` (
            ...
            NEW.`变压器编号`,
            CONCAT('配电房变压器-', v_配电房名称, '-', NEW.`变压器编号`),
            '变压器',
            '待补充型号',
            NOW(),
            10, -- 默认质保期10年
            NULL,
            CONCAT(DATE_FORMAT(NOW(), '%Y-%m-%d'), '/系统自动录入'),
            '正常使用',
            NOW(),
            NOW()
        );
...
INSERT INTO `告警信息表` VALUES ('ALM-PV-INV-101','设备故障','INV-101','2026-01-06 10:43:56','中','逆变器效率异常：INV-101效率80.00%<85%【已恢复：90.00%】','已结案','逆变器效率≥85%','2026-01-06 10:43:56',0,NULL,NULL,NULL,0,0,NULL),('ALM100','越限告警','PDF001','2026-01-04 07:30:00','中','变压器绕组温度偏高','未处理','绕组温度阈值:85℃','2026-01-04 07:30:00',0,NULL,NULL,NULL,0,0,NULL),('ALM101','设备故障','PDF002','2026-01-04 07:00:00','高','光伏逆变器故障 【告警已超15分钟未自动派单，请手动处理！】','未处理','发电量阈值:>0','2026-01-04 07:00:00',0,NULL,NULL,NULL,0,0,NULL),('ALM102','越限告警','PDF003','2026-01-04 06:45:00','低','回路电压接近上限','未处理','电压阈值:≤37kV','2026-01-04 06:45:00',0,NULL,NULL,NULL,0,0,NULL),('ALM103','设备故障','PDF004','2026-01-04 06:00:00','中','水表通讯故障','已结案','通讯状态:正常','2026-01-04 06:00:00',0,NULL,NULL,NULL,0,0,NULL),('ALM104','通讯故障','PDF005','2026-01-04 05:45:00','低','汇流箱通讯中断','处理中','通讯状态:正常','2026-01-04 05:45:00',0,NULL,NULL,NULL,0,0,NULL),('PV--100-01061040','设备故障','INV-100','2026-01-06 10:40:30','中','逆变器效率异常：INV-100效率80.00%<85%','未处理','逆变器效率≥85%','2026-01-06 10:40:30',0,NULL,NULL,NULL,0,0,NULL),('TEST-PROC-001','设备故障','INV-100','2026-01-06 10:23:31','中','测试工单分配-光伏','未处理',NULL,'2026-01-06 10:23:31',0,NULL,NULL,NULL,0,0,NULL);
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`jw`@`%`*/ /*!50003 TRIGGER `trg_回路新设备台账同步` AFTER INSERT ON `回路监测数据表` FOR EACH ROW BEGIN
    DECLARE v_配电房名称 VARCHAR(50);
    DECLARE v_exists INT DEFAULT 0;
...
CREATE TABLE `峰谷能耗数据表` (
  ...
  `峰谷电价` decimal(8,2) NOT NULL,
  `能耗成本` decimal(12,2) NOT NULL,
  PRIMARY KEY (`记录编号`),
  KEY `idx_厂区编号` (`厂区编号`),
  KEY `idx_统计日期` (`统计日期`),
  KEY `idx_厂区日期` (`厂区编号`,`统计日期`),
  CONSTRAINT `峰谷能耗数据表_chk_1` CHECK ((`尖峰时段能耗` >= 0)),
  CONSTRAINT `峰谷能耗数据表_chk_2` CHECK ((`高峰时段能耗` >= 0)),
  CONSTRAINT `峰谷能耗数据表_chk_3` CHECK ((`平段能耗` >= 0)),
  CONSTRAINT `峰谷能耗数据表_chk_4` CHECK ((`低谷时段能耗` >= 0)),
  CONSTRAINT `峰谷能耗数据表_chk_5` CHECK ((`峰谷电价` >= 0)),
  CONSTRAINT `峰谷能耗数据表_chk_6` CHECK ((`能耗成本` >= 0))
...
INSERT INTO `峰谷能耗数据表` VALUES ('FG100','电','真旺厂','2026-01-03',1500.25,2800.50,1200.75,800.30,6301.80,0.85,5356.53),('FG101','水','真旺厂','2026-01-03',25.50,45.25,18.75,12.30,101.80,3.50,356.30),('FG102','电','豆果厂','2026-01-03',1800.75,3200.25,1500.50,950.80,7452.30,0.85,6334.46),('FG103','蒸汽','A3厂区','2026-01-03',18.25,32.50,15.75,9.80,76.30,200.00,15260.00),('FG104','天然气','综合办公区','2026-01-03',85.20,120.50,65.30,42.80,313.80,3.20,1004.16);
...
CREATE TABLE `能耗监测数据表` (
  ...
  `待核实状态` varchar(2) NOT NULL DEFAULT '否',
  PRIMARY KEY (`数据编号`),
  ...
  CONSTRAINT `能耗监测数据表_chk_2` CHECK ((`待核实状态` in (_utf8mb4'是',_utf8mb4'否')))
...
INSERT INTO `能耗监测数据表` VALUES ('EM100','NH100','2026-01-04 08:00:00',25.50,'m³','优','真旺厂','否'),('EM101','NH100','2026-01-04 09:00:00',26.75,'m³','优','真旺厂','否'),('EM102','NH101','2026-01-04 08:00:00',120.80,'m³','优','真旺厂','否'),('EM103','NH102','2026-01-04 08:00:00',15.25,'t','优','豆果厂','否'),('EM104','NH103','2026-01-04 08:00:00',12.50,'m³','优','A3厂区','否');
...
/*!50106 DROP EVENT IF EXISTS `event_大屏数据每分钟刷新` */;
DELIMITER ;;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;;
...
/*!50106 CREATE*/ /*!50117 DEFINER=`jw`@`%`*/ /*!50106 EVENT `event_大屏数据每分钟刷新` ON SCHEDULE EVERY 1 MINUTE STARTS '2026-01-05 21:45:28' ON COMPLETION NOT PRESERVE ENABLE DO CALL `sp_大屏_实时数据刷新`() */ ;;
/*!50003 SET time_zone             = @saved_time_zone */ ;;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;;
/*!50003 SET character_set_client  = @saved_cs_client */ ;;
/*!50003 SET character_set_results = @saved_cs_results */ ;;
/*!50003 SET collation_connection  = @saved_col_connection */ ;;
/*!50106 DROP EVENT IF EXISTS `event_检查高等级告警延迟派单` */;;
DELIMITER ;;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;;
...
/*!50003 SET @saved_time_zone      = @@time_zone */ ;;
/*!50003 SET time_zone             = 'SYSTEM' */ ;;
/*!50106 CREATE*/ /*!50117 DEFINER=`jw`@`%`*/ /*!50106 EVENT `event_检查高等级告警延迟派单` ON SCHEDULE EVERY 1 MINUTE STARTS '2026-01-06 10:22:09' ON COMPLETION PRESERVE ENABLE DO BEGIN
    DECLARE v_告警编号 VARCHAR(20);
    DECLARE v_告警时间 DATETIME;
    DECLARE v_done INT DEFAULT FALSE;
    DECLARE v_seconds_diff INT;
    ...
    -- 5. 告警时间在15分钟内
    -- 6. 还没有创建过对应的延迟事件
    DECLARE cur_告警 CURSOR FOR
        SELECT `告警编号`, `告警时间`
        FROM `告警信息表`
        WHERE `告警等级` = '高' 
          AND `是否误报` = 0 
          AND `处理状态` = '未处理'
          AND (`自动派单状态` = 0 OR `自动派单状态` IS NULL)  -- 未尝试过自动派单
          AND `告警时间` > DATE_SUB(NOW(), INTERVAL 15 MINUTE)  -- 15分钟内的告警
          AND NOT EXISTS (
              SELECT 1 FROM INFORMATION_SCHEMA.EVENTS 
              WHERE EVENT_SCHEMA = DATABASE() 
              AND EVENT_NAME = CONCAT('event_delayed_dispatch_', REPLACE(`告警编号`, '-', '_'))
          );
    ...
    read_loop: LOOP
        FETCH cur_告警 INTO v_告警编号, v_告警时间;
        IF v_done THEN
            LEAVE read_loop;
        END IF;
        
        -- 计算距告警创建已经过了多少秒
        SET v_seconds_diff = TIMESTAMPDIFF(SECOND, v_告警时间, NOW());
        
        -- 检查是否已经过了15分钟（900秒）
        IF v_seconds_diff >= 900 THEN
            -- 如果已经超过15分钟，立即派单
            CALL `sp_执行延迟派单`(v_告警编号);
        ELSE
            -- 如果还没到15分钟，创建定时事件在15分钟时执行
            -- 计算还需要等待多少秒
            CALL `sp_创建精确延迟事件`(v_告警编号, v_告警时间);
        END IF;
    END LOOP;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;;
/*!50003 SET collation_connection  = @saved_col_connection */ ;;
/*!50106 DROP EVENT IF EXISTS `event_每周光伏模型优化提醒` */;;
DELIMITER ;;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;;
...
/*!50003 SET @saved_time_zone      = @@time_zone */ ;;
/*!50003 SET time_zone             = 'SYSTEM' */ ;;
/*!50106 CREATE*/ /*!50117 DEFINER=`jw`@`%`*/ /*!50106 EVENT `event_每周光伏模型优化提醒` ON SCHEDULE EVERY 1 WEEK STARTS '2026-01-06 03:00:00' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
    CALL sp_光伏预测偏差率统计与模型优化提醒();
END */ ;;
/*!50003 SET time_zone             = @saved_time_zone */ ;;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;;
/*!50003 SET character_set_client  = @saved_cs_client */ ;;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_光伏预测偏差率统计与模型优化提醒` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`jw`@`%` PROCEDURE `sp_光伏预测偏差率统计与模型优化提醒`()
BEGIN
    -- 声明变量：存储模型版本、连续异常天数、当前日期
    DECLARE v_model_version VARCHAR(20);
    DECLARE v_continuous_days INT DEFAULT 0;
    DECLARE v_current_date DATE;
    DECLARE v_end_date DATE;
    
    -- 声明游标：遍历所有存在有效预测数据的模型版本
    DECLARE model_cursor CURSOR FOR
        SELECT DISTINCT 预测模型版本
        FROM 光伏预测数据表
        WHERE 预测模型版本 IS NOT NULL
          AND 实际发电量kWh IS NOT NULL
          AND 偏差率 IS NOT NULL;
    
    -- 声明异常处理：游标遍历结束时退出
    DECLARE EXIT HANDLER FOR NOT FOUND CLOSE model_cursor;
    
    -- 初始化日期范围：统计当前日期往前30天内的数据（避免统计过久历史数据）
    SET v_end_date = CURDATE();
    SET v_current_date = DATE_SUB(v_end_date, INTERVAL 30 DAY);
    
    -- 打开游标遍历模型版本
    OPEN model_cursor;
    model_loop: LOOP
        FETCH model_cursor INTO v_model_version;
        
        -- 计算该模型版本在日期范围内，连续3天偏差率超15%的最大天数
        SELECT MAX(continuous_days) INTO v_continuous_days
        FROM (
            -- 子查询1：按日期统计该模型的日平均偏差率（绝对值）
            SELECT 
                预测日期,
                AVG(ABS(偏差率)) AS daily_avg_deviation,
                -- 子查询2：标记当日平均偏差率是否超15%
                IF(AVG(ABS(偏差率)) > 15, 1, 0) AS is_exceed_threshold,
                -- 子查询3：计算连续超阈值的天数（窗口函数实现）
                SUM(IF(AVG(ABS(偏差率)) > 15, 1, 0)) OVER (
                    PARTITION BY 预测模型版本
                    ORDER BY 预测日期
            ...
            FROM 光伏预测数据表
            WHERE 预测模型版本 = v_model_version
              AND 预测日期 BETWEEN v_current_date AND v_end_date
              AND 实际发电量kWh IS NOT NULL
              AND 偏差率 IS NOT NULL
            GROUP BY 预测日期
        ) AS daily_deviation;
        
        -- 校验：连续异常天数≥3天，生成模型优化提醒告警
        IF v_continuous_days >= 3 THEN
            -- 插入告警记录到告警信息表（与现有告警格式保持一致）
            INSERT INTO 告警信息表 (
                告警编号,
                告警类型,
                关联设备编号,
                ...
                -- 告警编号：模型优化提醒+模型版本+时间戳，确保唯一
                CONCAT('ALM-PV-MODEL-', v_model_version, '-', DATE_FORMAT(NOW(), '%y%m%d%H%i%S')),
                '模型优化提醒',  -- 告警类型：区分设备故障，明确为模型优化需求
                'SYSTEM-MODEL',  -- 关联设备编号：系统级告警，统一标识为模型模块
                NOW(),
                '中',  -- 告警等级：中等级，需数据分析师关注处理
                CONCAT('预测模型版本[', v_model_version, ']连续', v_continuous_days, '天偏差率超15%，请及时优化模型参数（如引入天气因子、更新训练数据）'),
                '未处理',
                '连续3天偏差率>15%'  -- 明确触发阈值，符合任务书要求
            );
        END IF;
        
        -- 重置变量，准备下一个模型版本遍历
        SET v_continuous_days = 0;
    END LOOP model_loop;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_创建精确延迟事件` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`jw`@`%` PROCEDURE `sp_创建精确延迟事件`(IN p_告警编号 VARCHAR(20), IN p_告警时间 DATETIME)
BEGIN
    DECLARE v_event_name VARCHAR(64);
    DECLARE v_seconds_to_wait INT;
    
    -- 计算需要等待的秒数（确保在告警创建后的第15分钟整执行）
    SET v_seconds_to_wait = 900 - TIMESTAMPDIFF(SECOND, p_告警时间, NOW());
    
    -- 确保等待时间在合理范围内（0-15分钟）
    IF v_seconds_to_wait BETWEEN 1 AND 900 THEN
        -- 生成事件名
        SET v_event_name = CONCAT('event_delayed_dispatch_', REPLACE(p_告警编号, '-', '_'));
        
        -- 标记为已创建事件
        UPDATE `告警信息表`
        SET `自动派单状态` = 3  -- 标记为"已创建延迟派单事件"
        WHERE `告警编号` = p_告警编号;
        
        -- 动态创建事件，在准确的15分钟时间点执行
        SET @create_event = CONCAT(
            'CREATE EVENT IF NOT EXISTS `', v_event_name, '` ',
            'ON SCHEDULE AT ''', DATE_ADD(p_告警时间, INTERVAL 15 MINUTE), ''' ',
            'ON COMPLETION NOT PRESERVE ',
            'ENABLE ',
            'DO ',
            'CALL `sp_执行延迟派单`(''', p_告警编号, ''')'
        );
        ...
        -- 记录日志
        INSERT INTO `系统日志表` (`日志内容`, `日志时间`, `日志级别`)
        VALUES (CONCAT('为告警创建延迟派单事件: ', p_告警编号, ', 将在', 
                DATE_ADD(p_告警时间, INTERVAL 15 MINUTE), '执行'), NOW(), 'INFO');
    END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_大屏_实时数据刷新` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`jw`@`%` PROCEDURE `sp_大屏_实时数据刷新`()
BEGIN
  DELETE FROM `实时汇总数据表` 
  WHERE `统计时间` < DATE_SUB(NOW(), INTERVAL 10 MINUTE);
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_执行延迟派单` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`jw`@`%` PROCEDURE `sp_执行延迟派单`(IN p_告警编号 VARCHAR(20))
BEGIN
    DECLARE v_告警等级 VARCHAR(20);
    DECLARE v_处理状态 VARCHAR(20);
    DECLARE v_是否误报 TINYINT;
    DECLARE v_自动派单状态 INT;
    DECLARE v_运维人员ID VARCHAR(20);
    DECLARE v_工单编号 VARCHAR(20);
    DECLARE v_告警时间 DATETIME;
    DECLARE v_seconds_diff INT;
    ...
    -- 获取告警信息
    SELECT `告警等级`, `处理状态`, `是否误报`, `自动派单状态`, `告警时间`
    INTO v_告警等级, v_处理状态, v_是否误报, v_自动派单状态, v_告警时间
    FROM `告警信息表`
    WHERE `告警编号` = p_告警编号;
    ...
    IF v_告警等级 = '高' 
       AND v_处理状态 = '未处理' 
       AND v_是否误报 = 0 
       AND v_seconds_diff >= 900
       AND (v_自动派单状态 != 1 OR v_自动派单状态 IS NULL) THEN
        
        -- 记录即将派单
        INSERT INTO `系统日志表` (`日志内容`, `日志时间`, `日志级别`)
        VALUES (CONCAT('开始执行15分钟延迟派单: ', p_告警编号, ', 告警时间: ', 
                v_告警时间, ', 已过去: ', v_seconds_diff, '秒'), NOW(), 'INFO');
        
        -- 寻找合适的运维人员（优化负载均衡）
        SELECT u.`用户ID` INTO v_运维人员ID
        FROM `用户表` u
        WHERE u.`角色` LIKE '%运维%'
          AND u.`状态` = '正常'  -- 只考虑状态正常的运维人员
          AND u.`用户ID` NOT IN (
              SELECT wo.`运维人员ID`
              FROM `运维工单数据表` wo
              WHERE wo.`复查状态` IN ('待复查', '未通过')
                AND wo.`派单时间` > DATE_SUB(NOW(), INTERVAL 4 HOUR)  -- 放宽时间范围
          )
        ORDER BY (
            -- 优先分配给待处理工单最少的人
            SELECT COUNT(*) 
            FROM `运维工单数据表` wo2 
            WHERE wo2.`运维人员ID` = u.`用户ID`
              AND wo2.`复查状态` = '待复查'
        ) ASC,
        (
            -- 再考虑今天的工作量
            ...
            FROM `运维工单数据表` wo2 
            WHERE wo2.`运维人员ID` = u.`用户ID`
              AND DATE(wo2.`派单时间`) = CURDATE()
        ) ASC,
        RAND()  -- 随机排序，避免总是分配给同一人
        LIMIT 1;
        
        -- 如果找到运维人员，创建工单
        IF v_运维人员ID IS NOT NULL THEN
            -- 生成工单编号
            SET v_工单编号 = CONCAT('WO-DLY-', DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'));
            
            -- 创建工单
            INSERT INTO `运维工单数据表` (
                `工单编号`, `告警编号`, `运维人员ID`, `派单时间`, 
                `复查状态`, `处理结果`, `延迟派单标识`
            ) VALUES (
                v_工单编号, p_告警编号, v_运维人员ID, NOW(),
                '待复查', '高等级告警，系统延迟15分钟后自动派单', 1
            );
            ...
            SET `处理状态` = '处理中',
                `自动派单状态` = 1,  -- 派单成功
                `最后派单时间` = NOW(),
                `派单尝试次数` = COALESCE(`派单尝试次数`, 0) + 1
            WHERE `告警编号` = p_告警编号;
            
            -- 记录成功日志
            INSERT INTO `系统日志表` (`日志内容`, `日志时间`, `日志级别`)
            VALUES (CONCAT('高等级告警15分钟延迟派单成功: ', p_告警编号, 
                    ' -> 工单:', v_工单编号, ' -> 运维:', v_运维人员ID), NOW(), 'INFO');
        ELSE
            -- 没有可用运维人员
            UPDATE `告警信息表`
            SET `自动派单状态` = 2,  -- 派单失败
            ...
            -- 记录失败日志
            INSERT INTO `系统日志表` (`日志内容`, `日志时间`, `日志级别`)
            VALUES (CONCAT('高等级告警延迟派单失败(无可用运维人员): ', p_告警编号), NOW(), 'WARN');
        END IF;
        
        -- 无论成功与否，都删除对应的事件（如果存在）
        SET @drop_event = CONCAT('DROP EVENT IF EXISTS `event_delayed_dispatch_', 
                                 REPLACE(p_告警编号, '-', '_'), '`');
        PREPARE stmt2 FROM @drop_event;
        EXECUTE stmt2;
    ...
    ELSE
        -- 记录不满足派单条件的情况（用于调试）
        INSERT INTO `系统日志表` (`日志内容`, `日志时间`, `日志级别`)
        VALUES (CONCAT('告警不满足15分钟延迟派单条件: ', p_告警编号, 
                ', 等级:', IFNULL(v_告警等级, 'NULL'), 
                ', 状态:', IFNULL(v_处理状态, 'NULL'),
                ', 误报:', IFNULL(v_是否误报, 'NULL'),
                ', 已过:', v_seconds_diff, '秒'), NOW(), 'DEBUG');
    END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
--
-- Final view structure for view `v_光伏日发电量统计`
--
...
/*!50001 VIEW `v_光伏日发电量统计` AS select cast(`pd`.`采集时间` as date) AS `统计日期`,`pd`.`设备编号` AS `设备编号`,`pi`.`设备类型` AS `设备类型`,`pi`.`安装位置` AS `安装位置`,`pd`.`并网点编号` AS `并网点编号`,`b`.`并网点名称` AS `并网点名称`,`b`.`并网电压等级` AS `并网电压等级`,sum(`pd`.`发电量kWh`) AS `当日总发电量kWh`,sum(`pd`.`自用电量kWh`) AS `当日总自用电量kWh`,sum(`pd`.`上网电量kWh`) AS `当日总上网电量kWh`,round(((sum(`pd`.`自用电量kWh`) / nullif(sum(`pd`.`发电量kWh`),0)) * 100),2) AS `自用电占比百分比`,max(`pd`.`采集时间`) AS `最新采集时间`,count(distinct `pd`.`数据编号`) AS `当日采集记录数`,`pi`.`运行状态` AS `运行状态`,(case when (`pi`.`设备类型` = '逆变器') then round(avg(`pd`.`逆变器效率`),2) else NULL end) AS `逆变器平均效率百分比` from ((`光伏发电数据表` `pd` left join `光伏设备信息表` `pi` on((`pd`.`设备编号` = `pi`.`设备编号`))) left join `并网点信息表` `b` on((`pd`.`并网点编号` = `b`.`并网点编号`))) group by cast(`pd`.`采集时间` as date),`pd`.`设备编号`,`pd`.`并网点编号`,`pi`.`设备类型`,`pi`.`安装位置`,`b`.`并网点名称`,`b`.`并网电压等级`,`pi`.`运行状态` order by `统计日期` desc,`当日总发电量kWh` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `v_光伏设备异常汇总`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`jw`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_光伏设备异常汇总` AS select `pi`.`设备编号` AS `设备编号`,`pi`.`设备类型` AS `设备类型`,`pi`.`安装位置` AS `安装位置`,`pi`.`装机容量kWp` AS `装机容量kWp`,`pi`.`投运时间` AS `投运时间`,`pi`.`运行状态` AS `运行状态`,`pi`.`通信协议` AS `通信协议`,(case when (`pi`.`运行状态` = '故障') then '设备故障' when (`pi`.`运行状态` = '离线') then '设备离线' when ((`pi`.`设备类型` = '逆变器') and (`pd`.`逆变器效率` < 85)) then '逆变器效率过低' else '其他异常' end) AS `异常类型`,(case when (`pi`.`运行状态` = '离线') then concat('离线时长约',(to_days(curdate()) - to_days(`pi`.`更新时间`)),'天') when ((`pi`.`设备类型` = '逆变器') and (`pd`.`逆变器效率` < 85)) then concat('当前效率',`pd`.`逆变器效率`,'%, 低于85%阈值') else '需现场排查具体故障' end) AS `异常详情`,`pd`.`并网点编号` AS `并网点编号`,`b`.`并网点名称` AS `并网点名称`,`pd`.`采集时间` AS `最新异常数据时间`,`pi`.`校准周期` AS `校准周期`,(to_days(curdate()) - to_days(`pi`.`投运时间`)) AS `运行天数`,coalesce((select `wo`.`处理结果` from (`运维工单数据表` `wo` join `告警信息表` `a` on((`wo`.`告警编号` = `a`.`告警编号`))) where (`a`.`关联设备编号` = `pi`.`设备编号`) order by `wo`.`处理完成时间` desc limit 1),'无历史维修记录') AS `历史维修记录` from ((`光伏设备信息表` `pi` left join `光伏发电数据表` `pd` on(((`pi`.`设备编号` = `pd`.`设备编号`) and (`pd`.`采集时间` = (select max(`光伏发电数据表`.`采集时间`) from `光伏发电数据表` where (`光伏发电数据表`.`设备编号` = `pi`.`设备编号`)))))) left join `并网点信息表` `b` on((`pd`.`并网点编号` = `b`.`并网点编号`))) where ((`pi`.`运行状态` in ('故障','离线')) or ((`pi`.`设备类型` = '逆变器') and (`pd`.`逆变器效率` < 85))) order by (case `pi`.`运行状态` when '故障' then 1 when '离线' then 2 else 3 end),`pd`.`采集时间` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
--
-- Final view structure for view `v_大屏_告警统计`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`jw`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_大屏_告警统计` AS select date_format(`a`.`发生时间`,'%Y-%m-%d %H:00:00') AS `统计小时`,count(0) AS `告警总次数`,count((case when (`a`.`告警等级` = '高') then 1 end)) AS `高等级告警数`,count((case when (`a`.`告警等级` = '中') then 1 end)) AS `中等级告警数`,count((case when (`a`.`告警等级` = '低') then 1 end)) AS `低等级告警数`,count((case when (`a`.`处理状态` = '未处理') then 1 end)) AS `未处理告警数`,count((case when (`a`.`处理状态` = '已处理') then 1 end)) AS `已处理告警数`,max(`a`.`发生时间`) AS `最近告警时间` from `告警信息表` `a` where (`a`.`发生时间` >= (now() - interval 24 hour)) group by date_format(`a`.`发生时间`,'%Y-%m-%d %H') order by `统计小时` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `v_大屏_能源总览`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`jw`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_大屏_能源总览` AS select `rsd`.`统计时间` AS `统计时间`,`rsd`.`总用电量kWh` AS `总用电量kWh`,`rsd`.`总用水量m3` AS `总用水量m3`,`rsd`.`总蒸汽消耗量t` AS `总蒸汽消耗量t`,`rsd`.`总天然气消耗量m3` AS `总天然气消耗量m3`,`rsd`.`光伏总发电量kWh` AS `光伏总发电量kWh`,`rsd`.`光伏自用电量kWh` AS `光伏自用电量kWh`,`rsd`.`总告警次数` AS `总告警次数`,`rsd`.`高等级告警数` AS `高等级告警数`,round((`rsd`.`光伏自用电量kWh` * 0.85),2) AS `光伏当日收益元` from `实时汇总数据表` `rsd` order by `rsd`.`统计时间` desc limit 1 */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `v_大屏_配电网状态`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`jw`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_大屏_配电网状态` AS select `s`.`配电房编号` AS `配电房编号`,`s`.`名称` AS `名称`,`s`.`电压等级` AS `电压等级`,`s`.`变压器数量` AS `变压器数量`,count(distinct `cd`.`回路编号`) AS `总回路数`,count((case when (`cd`.`数据状态` = '异常') then 1 end)) AS `异常回路数`,count((case when (`td`.`运行状态` = '异常') then 1 end)) AS `异常变压器数`,max(`cd`.`采集时间`) AS `最新数据时间`,(case when (count((case when (`cd`.`数据状态` = '异常') then 1 end)) > 0) then '存在异常' else '运行正常' end) AS `配电房状态` from ((`配电房信息表` `s` left join `回路监测数据表` `cd` on((`s`.`配电房编号` = `cd`.`配电房编号`))) left join `变压器监测数据表` `td` on((`s`.`配电房编号` = `td`.`配电房编号`))) group by `s`.`配电房编号`,`s`.`名称`,`s`.`电压等级`,`s`.`变压器数量` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
--
-- Final view structure for view `v_趋势_多周期对比`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`jw`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_趋势_多周期对比` AS select `htd`.`能源类型` AS `能源类型`,max((case when (`htd`.`统计周期` = '日') then `htd`.`能耗发电量数值` end)) AS `当日数值`,max((case when (`htd`.`统计周期` = '周') then `htd`.`能耗发电量数值` end)) AS `当周数值`,max((case when (`htd`.`统计周期` = '月') then `htd`.`能耗发电量数值` end)) AS `当月数值`,round(avg((case when (`htd`.`统计周期` = '日') then `htd`.`同比增长率` end)),2) AS `日均同比增长率`,round(avg((case when (`htd`.`统计周期` = '周') then `htd`.`环比增长率` end)),2) AS `周均环比增长率`,(case when (avg(`htd`.`同比增长率`) > 0) then '能耗上升' when (avg(`htd`.`同比增长率`) < 0) then '能耗下降' else '持平' end) AS `同比趋势结论` from `历史趋势数据表` `htd` where (`htd`.`统计时间` >= (curdate() - interval 30 day)) group by `htd`.`能源类型` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-01-06 10:52:12
