...
public interface EquipmentLedgerDao {
    ...
    @Select("SELECT `台账编号` AS ledgerId, `设备名称` AS equipmentName, `设备类型` AS equipmentType, " +
            "`型号规格` AS modelSpecification, `安装时间` AS installTime, `质保期` AS warrantyPeriod, " +
    ...
    @Select("SELECT `台账编号` AS ledgerId, `设备名称` AS equipmentName, `设备类型` AS equipmentType, " +
            ...
            "AND DATE_ADD(`安装时间`, INTERVAL `质保期` YEAR) BETWEEN CURDATE() " +
    ...
    @Update("UPDATE `设备台账数据表` SET `设备名称` = #{equipmentName}, " +
            ...
            "`安装时间` = #{installTime}, `质保期` = #{warrantyPeriod}, " +
