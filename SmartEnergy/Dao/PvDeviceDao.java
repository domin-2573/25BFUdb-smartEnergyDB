...
public interface PvDeviceDao {
    ...
    @Insert("INSERT INTO `光伏设备信息表` (`设备编号`, `设备类型`, `安装位置`, `装机容量kWp`, " +
            "`投运时间`, `校准周期`, `运行状态`, `通信协议`) " +
    ...
    @Select("SELECT `设备编号` AS deviceId, `设备类型` AS deviceType, `安装位置` AS installLocation, " +
            "`装机容量kWp` AS capacity, `投运时间` AS operationTime, `校准周期` AS calibrationCycle, " +
    ...
    @Select("SELECT `设备类型`, COUNT(*) as count, SUM(CAST(`装机容量kWp` AS DECIMAL(10,2))) as total_capacity " +
    ...
    @Select("SELECT `运行状态` as status, COUNT(*) as count, " +
            "ROUND(SUM(CAST(`装机容量kWp` AS DECIMAL(10,2))), 2) as total_capacity " +
    ...
    @Select("SELECT YEAR(`投运时间`) as year, COUNT(*) as device_count, " +
            "ROUND(SUM(CAST(`装机容量kWp` AS DECIMAL(10,2))), 2) as yearly_capacity " +
    ...
    @Update("<script>" +
            ...
            "<if test='capacity != null'>`装机容量kWp` = #{capacity},</if>" +
     ...
     */
    @Insert("<script>" +
            "INSERT INTO `光伏设备信息表` (`设备编号`, `设备类型`, `安装位置`, `装机容量kWp`, " +
    ...
    @Select("<script>" +
            ...
            "AND CAST(`装机容量kWp` AS DECIMAL(10,2)) >= #{minCapacity} " +
            "</if>" +
